...
class ArchetypeClassifier:
    ...
    def classify_build(
        ...
        secondary_archetype, secondary_score = (
        ...
        strengths, weaknesses = self._analyze_archetype_fit(
            characteristics, self.archetypes[primary_archetype]
        )

        # Generate recommendations
        recommendations = self._generate_archetype_recommendations(
            characteristics, self.archetypes[primary_archetype], weaknesses
        )
    ...
    def _extract_characteristics(
        self, character_data: Dict, dps: Optional[float], ehp: Optional[Dict[str, float]]
    ) -> Dict:
    ...
    def _calculate_archetype_score(
        self, characteristics: Dict, signature: ArchetypeSignature
    ) -> float:
        """
    ...
    def _analyze_archetype_fit(
        ...
        if dps > dps_ideal * 1.2:
            strengths.append(
                f"Exceptional DPS ({dps:,.0f}, {((dps/dps_ideal - 1) * 100):.1f}% above archetype average)"
            )
        ...
        if ehp > ehp_ideal * 1.3:
            strengths.append(
                f"Excellent survivability ({ehp:,.0f} EHP, {((ehp/ehp_ideal - 1) * 100):.1f}% above average)"
            )
    ...
    def _generate_archetype_recommendations(
        self, characteristics: Dict, signature: ArchetypeSignature, weaknesses: List[str]
    ) -> List[str]:
        ...
        for priority_stat in signature.priority_stats[:3]:  # Top 3 priorities
            if priority_stat == "dps" and characteristics["dps"] < signature.dps_range[2]:
                recommendations.append(
                    f"Increase DPS to reach {signature.dps_range[2]:,.0f} (archetype ideal)"
                )

            elif priority_stat == "ehp" and characteristics["ehp"] < signature.ehp_range[2]:
                recommendations.append(
                    f"Increase EHP to reach {signature.ehp_range[2]:,.0f} (archetype ideal)"
                )
        ...
        for weakness in signature.common_weaknesses:
            recommendations.append(
                f"Watch out for {weakness.replace('_', ' ')} (common archetype weakness)"
            )

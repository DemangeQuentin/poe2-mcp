...
class CharacterAnalyzer:
    ...
    def _analyze_defenses(self, character_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        life = defense_stats.get("life") or stats.get("life") or 0
        es = (
            defense_stats.get("energyShield")
            or stats.get("energyShield")
            or stats.get("energy_shield")
            or 0
        )
        ...
        ehp = (
            defense_stats.get("effectiveHealthPool")
            or stats.get("effectiveHealthPool")
            or raw_hp  # Fallback to raw HP if not calculated
        )

        # Resistances - handle multiple possible field names
        fire_res = (
            defense_stats.get("fireRes")
            or defense_stats.get("fireResistance")
            or stats.get("fireResistance")
            or 0
        )
        cold_res = (
            defense_stats.get("coldRes")
            or defense_stats.get("coldResistance")
            or stats.get("coldResistance")
            or 0
        )
        lightning_res = (
            defense_stats.get("lightningRes")
            or defense_stats.get("lightningResistance")
            or stats.get("lightningResistance")
            or 0
        )
        chaos_res = (
            defense_stats.get("chaosRes")
            or defense_stats.get("chaosResistance")
            or stats.get("chaosResistance")
            or 0
        )
    ...
    def _analyze_skills(self, character_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"total_skills": len(skills), "skill_groups": skills, "issues": []}

    def _analyze_gear(self, character_data: Dict[str, Any]) -> Dict[str, Any]:
        """Analyze equipped gear"""
        items = character_data.get("items", [])

        return {"total_items": len(items), "items": items, "issues": []}

    def _generate_recommendations(
        self, analysis: Dict[str, Any], character_data: Dict[str, Any]
    ) -> List[Dict[str, str]]:
